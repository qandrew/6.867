...
def updateThetaS(X,y,theta_old,theta_0_old,step,L):
    #update theta
    theta_new = np.zeros((X.shape[1],1)) #initiate theta at 0 #initiate theta_new to be 0s for now
    i = randint(0,X.shape[1]-1)
    for j in xrange(len(theta_old)):
        theta_new[j] = theta_old[j] + step*y[i,0]*X[i,j]*1/(1+np.exp(y[i,0]*(X[i]*theta_old+theta_0_old))) - step*L*theta_old[j]
    theta_0_new = theta_0_old + step*y[i,0]*1/(1+np.exp(y[i,0]*(X[i]*theta_old+theta_0_old)))
...
def gradientDescentStochastic(X,y, theta, theta_0, step, conv, L):
    #X: given X data
    #y: given y data
    #step: step size
    #conv: convergence criterion
    ...
    while abs(cost_old - cost_new) >= conv: #while we have not converged
        ...
        theta_new, theta_new_0, cost_new = updateThetaS(X,y,theta_old,theta_old_0,step,L)
        i += 1
        #print "iteration:", i, "cost:", cost_new, "diff:", cost_old - cost_new, "weights:", theta_new, theta_new_0
        if ((i-1)%200 == 0):
            print "iteration:", i-1, "cost:", cost_new, "weights:", theta_new, theta_new_0
        # print "theta", theta_new
        theta_0s.append(theta_new_0)
        theta_1s.append(theta_new[0,0])
        theta_2s.append(theta_new[1,0])
...
if __name__ == "__main__":
    ...
    theta_0 = 0
    step = 1 #specify step here
    conv = 0.0001 #specify convergence here

    # print cost(X,Y,[[-0.57],[6.322]],3.25,0)

    ### Lambda = 0 ###
    # L = 0
    # theta_f,theta_f_0, theta_0s, theta_1s, theta_2s = gradientDescentStochastic(X,Y,theta,theta_0,step,conv,L)
    ...
    L = 1
    print "Lambda", 1
    theta_f,theta_f_0, theta_0s, theta_1s, theta_2s = gradientDescentStochastic(X,Y,theta,theta_0,0.0001,conv,L)
    print "initial cost", cost(X,Y,theta,theta_0,L)
    print "final theta & cost", theta_f, theta_f_0, cost(X,Y,theta_f,theta_f_0,L)
