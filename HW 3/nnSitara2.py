...
if hw2_data:
    ...
    weights, offsets , acc, num_iters = NN_train(Xtrain, Ytrain, Xval, Yval, initial_rate=0.1, L=layers, M=HUDING, k=2)   
    print 'Finished training in ', num_iters, ' rounds with a validation accuracy of ', acc
    print 'Performance on test set: ', classify_accuracy(Xtest, Ytest, weights, offsets)
    ...
    # plot validation results
    print Xtest.shape, Ytest_values.shape, HUDING, 'data', name
    plot.plotDecisionBoundary(Xtest, Ytest_values, predictNN, [0], title = 'Set '+str(name)+' with '+
        str(layers-1)+' hidden layer(s), 1000,1000 neurons')
    pl.show()
